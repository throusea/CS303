...
class AI(object):
    ...
    def maximize(self, color, select_node, opcand_list, chessboard, depth, alpha, beta):
        ...
        if (ept_cnt > 8 and depth >= 5) or (len(candidate_list) == 0 and len(self.get_candidate_list(-color, chessboard)) == 0):
            return ((), self.eval(color, select_node, chessboard, opcand_list))
        if len(candidate_list) == 0 and len(self.get_candidate_list(-color, chessboard)) != 0:
            return self.minimize(-color, select_node, opcand_list, chessboard, depth, alpha, beta)
        maxNode, maxUtility = (), -inf
        for node in candidate_list:
            if depth == 1:
                _, utility = self.minimize(-color, node, opcand_list, self.play(color, node, chessboard), depth+1, alpha, beta)
                # print(chessboard, color, node, utility)
            else:
                _, utility = self.minimize(-color, select_node, opcand_list, self.play(color, node, chessboard), depth+1, alpha, beta)
    ...
    def minimize(self, color, select_node, opcand_list, chessboard, depth, alpha, beta):
        ...
        if (ept_cnt > 8 and depth >= 5) or (len(candidate_list) == 0 and len(self.get_candidate_list(-color, chessboard)) == 0):
            return ((), self.eval(color, select_node, chessboard, opcand_list))
        if len(candidate_list) == 0 and len(self.get_candidate_list(-color, chessboard)) != 0:
            return self.maximize(-color, select_node, opcand_list, chessboard, depth, alpha, beta)
        minNode, minUtility = (), inf
        for node in candidate_list:
            if depth == 2:
                _, utility = self.maximize(-color, select_node, candidate_list, self.play(color, node, chessboard), depth+1, alpha, beta)
            else:
                _, utility = self.maximize(-color, select_node, opcand_list, self.play(color, node, chessboard), depth+1, alpha, beta)
